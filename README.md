# ESP32_S3_Samples

.NET nanoFramework 演示微雪的 ESP32-S3-Zero 使用

板子介绍：[中文](https://www.waveshare.net/wiki/ESP32-S3-Zero) | [English](https://www.waveshare.com/wiki/ESP32-S3-Zero)

因硬件的特异性，项目均以 `ESP32-S3-Zero` 为例，其他开发板请自行修改一些引脚等配置。

# 案例说明

1. OLED_SSD13XX： OLED，I2C显示屏
2. BoardButton：板载用户按钮

# 相关文章

1. [《使用.Net nanoFramework 驱动ESP32的OLED显示屏》](https://mp.weixin.qq.com/s/v1INAkffHlCpHLCDf5xBtA)
1. [《使用.Net nanoFramework为ESP32连接Wi-Fi网络》](https://mp.weixin.qq.com/s/E5eaRHS1XKBcrhzphiN3iQ)
1. [《ESP32点阵字体及.NET字体制作工具》](https://mp.weixin.qq.com/s/SLHfHxYIA4vm-Z55LOgjgw)
1. [《使用.Net nanoFramework为ESP32进行蓝牙配网》](https://mp.weixin.qq.com/s/X3L0uZUAEOEthQDnUJLPsg)
1. [《使用.Net nanoFramework获取ESP32板载按键的点击事件》](https://mp.weixin.qq.com/s/7cpsjXx8K7OxVXRm_hZxqg)

<p align="center">
更多文章可关注我的微信公众号：桑榆肖物 <br/>
<img src="https://open.weixin.qq.com/qr/code?username=gh_c874018d0317" />
</p>

# 相关项目

1. [ESP32_S2_PICO](/sangyuxiaowu/ESP32_S2_PICO)